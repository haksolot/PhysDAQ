...
static void enter_sleep(void)
{
	...
	/* Close the session file BEFORE cutting power. System Off is
	 * indistinguishable from a power cut as far as the card is concerned,
	 * so anything still buffered in the storage thread or in the FAT layer
	 * would be lost — and unlike a real power cut, this one is voluntary
...
void power_update(const struct sensor_value gyro[3])
{
	...
	/* Gated off on the hub (CONFIG_PHYSDAQ_DEEP_SLEEP=n). A hub records
	 * unattended to microSD and is routinely stationary and unworn on a
	 * bench — exactly the state this timer reads as "put down" — so System
	 * Off here would cut a recording short. The idle accounting above still
	 * runs and still shows on the status line, so the countdown remains
	 * observable; only the shutdown is suppressed. */
	if (IS_ENABLED(CONFIG_PHYSDAQ_DEEP_SLEEP) &&
	    (now - last_active_ms) >= IDLE_TIMEOUT_MS) {
		enter_sleep();
	}
}
